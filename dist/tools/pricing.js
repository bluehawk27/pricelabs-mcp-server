...
Examples:
  ...
  - Get prices with reasons: { listings: [{ id: "123", pms: "airbnb", reason: true }] }`,
        ...
        try {
            const result = await makeApiRequest("listing_prices", "POST", { listings: params.listings });
...
Examples:
  - Get rate plans: { listing_id: "123", pms_name: "airbnb" }`,
        ...
        try {
            const result = await makeApiRequest("fetch_rate_plans", "GET", undefined, { listing_id: params.listing_id, pms_name: params.pms_name });
            return {
