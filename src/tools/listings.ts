...
import type { Listing, ListingUpdate } from "../types.js";
...
Examples:
  ...
  - Get all visible listings: { skip_hidden: true }`,
    ...
    async (params) => {
      try {
        const listings = await makeApiRequest<Listing[]>("listings", "GET", undefined, {
...
Examples:
  - Get listing details: { listing_id: "12345" }`,
    ...
    async (params) => {
      try {
        const listing = await makeApiRequest<Listing>(
          `listings/${encodeURIComponent(params.listing_id)}`
        );
        return {
...
Examples:
  ...
  - Update tags: { listings: [{ id: "123", pms: "airbnb", tags: ["beachfront", "luxury"] }] }`,
    ...
    async (params) => {
      try {
        const result = await makeApiRequest<unknown>(
          "listings",
          "POST",
          { listings: params.listings as ListingUpdate[] }
        );
        return {
