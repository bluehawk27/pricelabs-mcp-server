...
Examples:
  - Get overrides: { listing_id: "123", pms: "airbnb" }`,
    ...
    async (params) => {
      try {
        const result = await makeApiRequest<{ overrides: Override[] }>(
          `listings/${encodeURIComponent(params.listing_id)}/overrides`,
          "GET",
          undefined,
          { pms: params.pms }
        );
  ...
  - Delete overrides: { listing_id: "123", pms: "airbnb", dates: ["2025-12-31", "2025-12-30"] }`,
    ...
    async (params) => {
      try {
        ...
        return {
          content: [{ type: "text" as const, text: "Overrides deleted successfully." }],
        };
      } catch (error) {
        return { content: [{ type: "text" as const, text: handleApiError(error) }] };
      }
